...
%% '____' 2.0 beats
%% '_____' 4.0 beats
-type beats() :: '_' | '__' | '___' | '____' | '_____'.
...
%% effect of sound 
-type effect() :: reverb | echo | wobble | slicer | distortion | ping_pong | record | no_effect.

%%instrument for sound
-type instrument() :: string().

%%play mode for the track
...
-callback pitch_based_track() -> [{name(), mode(), duration(), delay(), instrument(), sound(), effect(), envelope()}].
...
-callback sample_pattern_track() -> [{name(), mode(), duration(), delay(), sample(), pattern(), effect(), envelope()}].


%%  duration of first event
-type first() :: '_' | '__' | '___' | '____' | '_____'.

%% duration of last event
-type last() :: '_' | '__' | '___' | '____' | '_____'.
...
%% the lists of note, beats for randomly choosing 
-callback choose_play() -> [{name(), mode(), delay(), instrument(), [note()], [beats()]}].

                   %% 3         3       3          3            4        4       3      3
-type chord_type() :: major | minor | augmented | diminished | major7 | minor7 | sus2 | sus4.


-callback chord_play() -> [{name(), mode(), delay(), instrument(), note(), chord_type(), [beats()], effect(), envelope()}].
